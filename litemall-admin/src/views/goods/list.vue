<template>
  <div class="app-container">
    ...
    <div class="filter-container">
      ...
      <el-button class="filter-item" type="primary" icon="el-icon-edit" @click="handleCreate">{{ $t('app.button.create')
        }}
      </el-button>
      <el-button :loading="downloadLoading" class="filter-item" type="primary" icon="el-icon-download"
                 @click="handleDownload">{{ $t('app.button.download') }}
      </el-button>
      <el-button class="filter-item" type="danger" icon="el-icon-delete" :disabled="batchDeleteArr.length === 0"
                 @click="handleDeleteRows">{{ $t('app.button.batch_delete') }}
      </el-button>
    </div>
    ...
    <el-table v-loading="listLoading" :data="list" :element-loading-text="$t('app.message.list_loading')" border fit
      ...
      <el-table-column align="center" property="iconUrl" :label="$t('goods_list.table.pic_url')">
        <template slot-scope="scope">
          <el-image :src="thumbnail(scope.row.picUrl)" :preview-src-list="toPreview(scope.row, scope.row.picUrl)"
      ...
      <el-table-column align="center" :label="$t('goods_list.table.detail')" prop="detail">
        <template slot-scope="scope">
          ...
          <el-button type="primary" size="mini" @click="showDetail(scope.row.detail)">{{ $t('app.button.view') }}
          </el-button>
        </template>
      </el-table-column>
      ...
      <el-table-column align="center" :label="$t('goods_list.table.is_new')" prop="isNew">
        <template slot-scope="scope">
          <el-tag :type="scope.row.isNew ? 'success' : 'error' ">{{ $t(scope.row.isNew ? 'goods_list.value.is_new_true'
      ...
      <el-table-column align="center" :label="$t('goods_list.table.is_hot')" prop="isHot">
        <template slot-scope="scope">
          <el-tag :type="scope.row.isHot ? 'success' : 'error' ">{{ $t(scope.row.isHot ? 'goods_list.value.is_hot_true'
      ...
      <el-table-column align="center" :label="$t('goods_list.table.is_on_sale')" prop="isOnSale">
        <template slot-scope="scope">
          <el-tag :type="scope.row.isOnSale ? 'success' : 'error' ">{{ $t(scope.row.isOnSale ?
            'goods_list.value.is_on_sale_true' : 'goods_list.value.is_on_sale_false') }}
          </el-tag>
        </template>
      </el-table-column>
